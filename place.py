    ...
    def buy(self, player):
        ...
        while True:
            ...
            for i, item_name in enumerate(item_list, start=1):
                ...
                say(f"{i}) {item_name}: {info['price']}원, HP +{info['HP_gain']}")
            say(f"{len(item_list) + 1}) 종료")
            ...
            say(f"{item_name}을(를) 구매해서 가방에 넣었다. 계좌 잔액 = {player.balance}원")
    
    def sell(self, player):
        if not self.sell_prices:
            say("이곳에서는 판매할 수 없습니다.")
            return
        ...
        while True:
            ...
            for i, item_name in enumerate(item_list, start=1):
                say(f"{i}) {item_name} x{sellable[item_name]}")
            say(f"{len(item_list) + 1}) 종료")
            ...
            player.balance += price
            say(f"{item_name}를 판매해서 {price}원을 벌었다. 계좌 잔액 = {player.balance}원")
    ...
    def handle_quest(self, player):
        ...
        if self.quest_role == "give_intro":
            if (intro_name not in player.quests 
                ...
                say(quests[intro_name].description)
                player.quests.append(intro_name)
                say("[임무목록]에 임무가 추가되었습니다.")
            else:
                say("이미 독수리상으로 가는 임무를 받았습니다.")
            return
        ...
        if self.quest_role == "give_quests":
            if intro_name in player.quests:
                ...
                say(f"다음의 임무가 해결되었다! [{quests[intro_name].description}]")
        ...
        if self.quest_role in ("report_bujori", "report_wisaeng"):
            ...
            if answer == quest_answers[quest_name]:
                ...
                say(f"다음의 임무가 해결되었다! [{quest_name}]")
                say("수업들으러 이윤재관 가야지!")
            else:
                say("답이 틀렸습니다.")
            return
        ...
        if self.quest_role == "final":
            ...
            if bujori_done and wisaeng_done:
                say("부조리와 식중독 수사를 완료했구나! 수업은 이걸로 끝입니다. 또 만나요~")
                return "GAME_OVER"
            
            if bujori_done:
                say("부조리 수사를 완료했구나! 식중독 원인도 찾아주세요~")
            elif wisaeng_done:
                say("식중독 수사를 완료했구나! 부조리도 찾아주세요~")
            else:
                say("아직 수사를 완료하지 못했네요. 본관과 세브란스에 가서 보고하세요.")
            return
